...
class OrderAcceptedScreen extends React.Component {
  constructor(props) {
    ...
    const { order, brokerConnection, orderFinalAmount } = props;
    this.state = {
      order: order,
  ...
  componentWillReceiveProps(newProps) {
    // Logger.log({ name: 'OrderAcceptedScreen.componentWillReceiveProps()', newProps: newProps })
  }

  componentWillMount() {
    // Logger.log({ name: 'OrderAcceptedScreen.componentWillMount()' })
    // firebase.analytics().setCurrentScreen('order_' + this.state.order.offering.tickerSymbol + '_congratulations')

  }

  componentWillUnmount() {
    // Logger.log({ name: 'OrderAcceptedScreen.componentWillUnmount()' })
  }
  ...
  render() {
    const { order, brokerConnection, orderFinalAmount } = this.state
    const { offering } = order

    const brokerDealerName = brokerConnection.brokerDealerName
    const orderAmount = '$' + numberWithCommas(orderFinalAmount.requested_amount)

    const baseCalPrice = offering.finalPrice || offering.maxPrice

    const approximateShares = (Math.floor(orderFinalAmount.requested_amount / baseCalPrice)) ? Math.floor(orderFinalAmount.requested_amount / baseCalPrice) : 0
    ...
    const orderDescription1 = 'Up to ' + orderAmount + ' worth of ' + offering.name
