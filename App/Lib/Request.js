...
export default function request(tokens, url, {method, headers: {'Content-Type': type = contentType.FORM, ...headers} = {}, body = {}, params = {}, ...init}, extraParams = {}) {
  const requestParams = {...method === 'POST' && type === contentType.FORM ? body : {}, ...params}
  const queryString = query(params)

  // Logger.log({ function: 'Request.request', requestParams: requestParams, queryString: queryString, url: url })
  ...
  .then(response => {
    // Logger.log({ function: 'Request.request', response: response })
    return response
  })
  .then(response =>
    response.ok ?
  ...
  .catch((error) => {
    // Logger.log({ function: 'Request.request', error: error })
  })
